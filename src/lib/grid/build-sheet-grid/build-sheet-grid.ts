...
import { MAX_COLS, MAX_ROWS } from '@/lib/grid/build-sheet-grid/constants';
...
export function buildSheetGrid(workbook: XLSX.WorkBook, sheetName: string): SheetGrid {
  ...
  const columns = buildColumns(sheet, range.s.c, endCol);
  const visibleRows = buildVisibleRows(sheet, range.s.r, endRow);
